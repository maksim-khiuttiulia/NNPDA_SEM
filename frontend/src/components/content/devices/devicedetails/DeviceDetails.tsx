...
import {createSensor, deleteDevice, getDevice, getSensors} from "../../../../services/DeviceService";
...
class DeviceDetails extends Component<Props, State> {
    ...
    loadData = () => {
        ...
        let devicePromise = getDevice(id);
        let sensorsPromise = getSensors(id);
    ...
    private onAddSensorSubmit = (sensor: Sensor): void => {
        ...
        createSensor(sensor, this.state.device.id).then(data => {
            this.loadData();
        }).catch(reason => {
    ...
    private onDeleteSubmit = (): void => {
        ...
        deleteDevice(id).then(resp => {
            this.props.history.push(RouterConstants.devices);
        }).catch(reason => {
            this.setState({isError: true, isLoading: false})
        })
    }
...
export default withTranslation()(withRouter(DeviceDetails))