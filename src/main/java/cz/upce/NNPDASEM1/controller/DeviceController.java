...
public class DeviceController extends ControllerAncestor {
    ...
    @RequestMapping(value = "/{id}/sensors/{sensorId}", method = RequestMethod.GET)
    public SensorDto getSensors(@PathVariable("id") Long id, @PathVariable("sensorId") Long sensorId) {
        User user = getCurrentUser();
        Device device = deviceService.getDevice(id, user);
